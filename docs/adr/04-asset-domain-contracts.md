...
| `searchByFileName(AccountId $accountId, string $query, AssetStatus $status, int $offset, int $limit): array` | Return only assets for the supplied account and exact `status` whose `fileName` contains the trimmed query as a plain-text, case-insensitive substring. Results are ordered by `createdAt` descending, then `id` ascending, and then bounded using zero-based `offset` and `limit`. Return an empty list when the trimmed query is empty or when `offset` exceeds the total count. | Search flows need deterministic account-scoped matching with repository-owned pagination instead of loading extra rows into PHP. |
| `countByFileName(AccountId $accountId, string $query, AssetStatus $status): int`                             | Return the count of assets that match the same account, status, and plain-text file-name search semantics. Return `0` when the trimmed query is empty.                                                                                                                                                                                                                             | Search flows need pagination metadata without counting filtered rows in application code.                                        |

Persistence must store `accountId`, `fileName`, `mimeType`, and uploaded-state `completionProof` values without imposing arbitrary length limits (such as a VARCHAR(255) ceiling). In this chunk the domain still models those fields as trimmed non-empty text, so persistence must preserve the full value it receives.
...
- `searchByFileName(...)` was refined so repository implementations own status scoping and bounded pagination for account-scoped file-name search.
- `countByFileName(...)` was added to the repository contract so search can compute page metadata without counting filtered rows in application code.
